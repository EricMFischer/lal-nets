...
Tr = theano.tensor.nlinalg.trace
Diag = theano.tensor.nlinalg.ExtractDiag()
Range = theano.tensor.ptp
Tensordot = theano.tensor.tensordot
Scan = theano.scan
...
class AcolRegularizerNull(Regularizer):
    ...
    def __call__(self, x):
        ...
        if self.balance_type < 5:
            ...
            Z_bar = Z * K.cast(Z>0., K.floatx())
            U = K.dot(K.transpose(Z_bar), Z_bar) * mask
